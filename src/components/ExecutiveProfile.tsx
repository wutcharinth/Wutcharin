        ...
        <div ref={ref} className="grid grid-cols-1 lg:grid-cols-12 gap-x-12 gap-y-6 items-baseline">
            <div className="lg:col-span-7">
                <div className="flex items-baseline gap-3 mb-2">
                    <span className="font-mono text-[10px] text-text-strong/30 tracking-[0.28em] uppercase">01</span>
                    <span className="font-mono text-[10px] text-signal/70 tracking-[0.28em] uppercase">{metric.unit}</span>
                </div>
                <div className="font-medium tracking-[-0.05em] leading-[0.85] text-text-strong tabular-nums text-[clamp(5rem,12vw,11rem)]">
                    {count >= 1000 ? `${(count / 1000).toFixed(0)}k+` : `${count}+`}
                </div>
                <p className="mt-4 max-w-md text-base md:text-lg text-text-strong/60 font-light leading-relaxed">
                    {metric.label.toLowerCase()}.
                </p>
            </div>
            <div className="lg:col-span-5 lg:pt-12 space-y-3">
                <p className="text-sm md:text-base text-text-strong/50 font-light leading-relaxed max-w-md">
                    {metric.detail}
                </p>
                <ul className="font-mono text-[10px] uppercase tracking-[0.28em] text-text-strong/40 space-y-1.5">
                    {metric.breakdown.map((b) => (
                        <li key={b.name} className="flex items-center justify-between gap-4 max-w-xs">
                            <span>{b.name}</span>
                            <span className="text-signal/70 tabular-nums">{b.pct}%</span>
                        </li>
                    ))}
                </ul>
            </div>
...
function CompactMetric({
    ...
    return (
        <div
            ref={ref}
            className="group relative py-5 border-t border-text-strong/[0.06] hover:border-text-strong/15 transition-colors cursor-default"
            onMouseEnter={onHover}
        >
            <div className="grid grid-cols-12 gap-4 items-baseline">
                <span className="col-span-1 font-mono text-[10px] text-text-strong/25 tracking-[0.28em] tabular-nums">
                    0{index + 2}
                </span>
                <div className="col-span-7 sm:col-span-8">
                    <p className="text-base md:text-xl text-text-strong/85 font-light tracking-tight leading-snug">
                        <span className="font-medium text-text-strong tabular-nums">{formatBig(count)}</span>
                        <span className="text-text-strong/50"> {metric.label.toLowerCase()}</span>
                    </p>
                </div>
                <span className="col-span-4 sm:col-span-3 text-right font-mono text-[10px] text-text-strong/35 tracking-[0.28em] uppercase">
                    {metric.unit}
                </span>
            </div>
                ...
                {active && (
                    ...
                    >
                        <div className="pt-4 pl-[8.33%] grid grid-cols-12 gap-4">
                            <p className="col-span-12 sm:col-span-8 text-sm text-text-strong/55 font-light leading-relaxed max-w-xl">
                                {metric.detail}
                            </p>
                            <ul className="col-span-12 sm:col-span-4 font-mono text-[10px] uppercase tracking-[0.28em] text-text-strong/40 space-y-1">
        ...
        <section id="about" className="relative py-32 px-4 overflow-hidden">
            ...
            <div className="relative max-w-5xl mx-auto">
                <motion.div
                    ...
                    className="inline-flex items-center gap-2 mb-8 px-3 py-1 rounded-full border border-text-strong/10"
                >
                    <span className="h-1 w-1 rounded-full bg-violet-400" />
                    <span className="text-[10px] font-mono uppercase tracking-[0.28em] text-text-strong/50">02 · Profile</span>
                </motion.div>
                ...
                <SplitText
                    ...
                    mode="words"
                    stagger={0.05}
                    className="text-4xl md:text-6xl font-medium tracking-[-0.03em] text-text-strong leading-[0.95] max-w-3xl mb-8"
                >
                    Executive leader at the intersection of strategy and code.
                </SplitText>

                <RevealOnScroll staggerChildren={0.1} duration={0.8}>
                    <p data-reveal-child className="max-w-2xl text-base md:text-lg text-text-strong/50 font-light leading-relaxed mb-6">
                    ...
                    <p data-reveal-child className="max-w-2xl text-base md:text-lg text-text-strong/50 font-light leading-relaxed mb-24">
                        Not just agents, but the <span className="text-text-strong/70">systems around them</span>: orchestration, memory, tool surfaces, guardrails, and evaluation that make multi-step agentic workflows reliable in production, not just in the demo.
                    </p>
                </RevealOnScroll>

                <div className="mb-12 flex items-baseline justify-between flex-wrap gap-4">
                    <h3 className="text-xs font-mono uppercase tracking-[0.28em] text-text-strong/40">
                        Measured Impact
                    </h3>
                    <div className="flex items-center gap-2 text-[10px] font-mono uppercase tracking-[0.28em] text-text-strong/30">
                        <span className="h-1.5 w-1.5 rounded-full bg-text-strong/30" />
                ...
                <div
                    className="mt-16 border-b border-text-strong/[0.06]"
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-[auto_1fr] gap-10 md:gap-14 items-end">
                        <div data-reveal-child className="relative">
                            ...
                            <div className="relative h-44 w-44 md:h-56 md:w-56 overflow-hidden">
                                ...
                                <div aria-hidden="true" className="absolute inset-0 bg-bg/25 mix-blend-multiply" />
                            </div>
                            <div className="mt-4 font-mono text-[10px] uppercase tracking-[0.28em] text-signal/70">
                                Bangkok · 2026
                            </div>
                            <div className="mt-1 text-sm font-medium text-text-strong/85 tracking-[-0.005em]">
                                Wutcharin Thatan
                            </div>
                        </div>
                        <div className="max-w-2xl">
                            <div data-reveal-child className="font-mono text-[10px] uppercase tracking-[0.28em] text-signal/70 mb-4">
                                Operating principle
                            </div>
                            <p data-reveal-child className="text-2xl md:text-4xl font-light text-text-strong/90 tracking-[-0.01em] leading-[1.2]">
