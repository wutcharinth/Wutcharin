...
const ProjectNavigation = ({ currentId }: ProjectNavigationProps) => {
    ...
    return (
        <section className="py-20 border-t border-text-strong/[0.06]">
            <div className="max-w-6xl mx-auto px-6">
                <div className="flex items-baseline justify-between mb-8 gap-6 flex-wrap">
                    ...
                    <Link
                        to="/"
                        className="font-mono text-[10px] uppercase tracking-[0.28em] text-text-mute hover:text-text-strong flex items-center gap-2 transition-colors"
                    >
                ...
                <div className="border-b border-text-strong/[0.06]">
                    {recommendedProjects.map((project) => (
                        <Link
                            key={project.id}
                            to={project.link}
                            className="group grid grid-cols-12 gap-x-4 items-center border-t border-text-strong/[0.06] py-4 px-2 -mx-2 transition-colors hover:bg-text-strong/[0.025]"
                        >
                            ...
                            <span className="hidden sm:block sm:col-span-6 font-mono text-[10px] uppercase tracking-[0.18em] text-text-strong/35 truncate">
                                {project.tagline}
                            </span>
                            <span className="col-span-1 flex justify-end">
                                <project.icon className="w-3.5 h-3.5 text-text-strong/25 transition-colors group-hover:text-text-strong" aria-hidden="true" />
                            </span>
                        </Link>
                    ))}
                </div>
